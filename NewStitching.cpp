...
void NewStitching::createCamera()
{
  ...
  mCamera->setPosition(Ogre::Vector3(0, 0, 270));
...
{	
	...
	{
		Patch* patch = new Patch(false, numberOfPatch, position, mSceneMgr);
		position = position+20;																			//increase the position for the next patch
